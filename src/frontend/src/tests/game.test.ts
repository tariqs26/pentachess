...
vi.mock("../../piece/utils", () => ({
  getPossibleMoves: vi.fn().mockReturnValue(new Set()),
  getInvalidMoves: vi.fn().mockReturnValue(new Set()),
  canPromote: vi.fn().mockReturnValue(false),
}))

// Mock moveHelper in utils
vi.mock("../utils", () => {
  const originalModule = vi.importActual("../utils") as any
  ...
  return {
    ...
    moveHelper: vi.fn().mockImplementation(() => ({
      turn: "b",
      status: "playing",
      checkedColor: null,
      move: {} as Move,
    })),
    createGameState: vi.fn().mockReturnValue({
